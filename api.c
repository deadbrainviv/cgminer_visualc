		...
		switch(type) {
			...
			case API_BOOL:
				...
				*((bool *)(api_data->data)) = *((bool *)data);
				break;
			case API_timeval2:
				api_data->data = (void *)malloc(sizeof(struct timeval2));
				memcpy(api_data->data, data, sizeof(struct timeval2));
				break;
			case API_TIME:
...
struct api_data *api_add_timeval2(struct api_data *root, char *name, struct timeval2 *data, bool copy_data)
{
	return api_add_data_full(root, name, API_timeval2, (void *)data, copy_data);
}
	...
	*buf = '\0';
	...
	while (root) {
		...
		switch(root->type) {
			...
			case API_timeval2:
				sprintf(buf, "%ld.%06ld",
					((struct timeval2 *)(root->data))->tv_sec,
					((struct timeval2 *)(root->data))->tv_usec);
				break;
			case API_TEMP:
...
static void gpustatus(struct io_data *io_data, int gpu, bool isjson, bool precom)
{
	...
	if (gpu >= 0 && gpu < nDevs) {
		...
		last_share_pool = cgpu->last_share_pool_time > 0 ? cgpu->last_share_pool : -1;
...
static void poolpriority(struct io_data *io_data, __maybe_unused SOCKETTYPE c, char *param, bool isjson, __maybe_unused char group)
{
	...
	pools_changed =(bool*) malloc(total_pools*sizeof(bool));
	new_prio=(int*) malloc(total_pools*sizeof(int));
...
static int itemstats(struct io_data *io_data, int i, char *id, struct cgminer_stats *stats, struct cgminer_pool_stats *pool_stats, struct api_data *extra, bool isjson)
{
	...
	root = api_add_timeval2(root, "Wait", &(stats->getwork_wait), false);
	root = api_add_timeval2(root, "Max", &(stats->getwork_wait_max), false);
	root = api_add_timeval2(root, "Min", &(stats->getwork_wait_min), false);
	...
	if (pool_stats) {
		...
		root = api_add_timeval2(root, "Pool Wait", &(pool_stats->getwork_wait), false);
		root = api_add_timeval2(root, "Pool Max", &(pool_stats->getwork_wait_max), false);
		root = api_add_timeval2(root, "Pool Min", &(pool_stats->getwork_wait_min), false);
...
static void minecoin(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	...
	if (current_fullhash && *current_fullhash) {
		root = api_add_timeval2(root, "Current Block Time", &block_timeval, true);
		root = api_add_string(root, "Current Block Hash", current_fullhash, true);
	} else {
		struct timeval2 t = {0,0};
		root = api_add_timeval2(root, "Current Block Time", &t, true);
...
static void dozero(struct io_data *io_data, __maybe_unused SOCKETTYPE c, char *param, bool isjson, __maybe_unused char group)
{
	...
	*sum = tolower(*sum);
	...
	if (dosum)
		message(io_data, MSG_ZERSUM, 0, ball ? "All" : "BestShare", isjson);
	else
		message(io_data, MSG_ZERNOSUM, 0, ball ? "All" : "BestShare", isjson);
}
...
static void send_result(struct io_data *io_data, SOCKETTYPE c, bool isjson)
{
	...
	struct timeval timeout = {0, 50000}; // allow 50ms per attempt
	fd_set wd;
	...
	while (count++ < 5 && tosend > 0) {
		FD_ZERO(&wd);
		FD_SET(c, &wd);
		if ((res = select(c + 1, NULL, (__inout_opt fd_set FAR *)&wd, NULL, &timeout)) < 1) {
