...
static struct timeval2 total_tv_start, total_tv_end;
...
void get_datestamp(char *f, struct timeval2 *tv)
{
	struct tm *tm;
...
void get_timestamp(char *f, struct timeval2 *tv)
{
	struct tm *tm;
...
static void calc_midstate(struct work *work)
{
	...
	sha2_update((sha2_context*)&ctx, data.c, 64);
...
static bool submit_upstream_work(struct work *work, CURL *curl, bool resubmit)
{
	...
	struct pool *pool = work->pool;
	int rolltime;
	struct timeval2 tv_submit, tv_submit_reply;
	char hashshow[64 + 4] = "";
	char worktime[200] = "";
	...
	if (!QUIET) {
		...
		if (opt_worktime) {
			...
			double getwork_time = tdiff((struct timeval2 *)&(work->tv_getwork_reply),
							(struct timeval2 *)&(work->tv_getwork));
			double getwork_to_work = tdiff((struct timeval2 *)&(work->tv_work_start),
							(struct timeval2 *)&(work->tv_getwork_reply));
			double work_time = tdiff((struct timeval2 *)&(work->tv_work_found),
							(struct timeval2 *)&(work->tv_work_start));
			double work_to_submit = tdiff(&tv_submit,
							(struct timeval2 *)&(work->tv_work_found));
			...
			if (work->clone) {
				sprintf(workclone, "C:%1.3f",
					tdiff((struct timeval2 *)&(work->tv_cloned),
						(struct timeval2 *)&(work->tv_getwork_reply)));
			}
			else
				strcpy(workclone, "O");
...
static void calc_diff(struct work *work, int known)
{
	...
	if (opt_scrypt) {
		...
		work->work_difficulty = DIFFEXACTONE / (targ ? targ : DIFFEXACTONE);
	} else
...
static void get_benchmark_work(struct work *work)
{
	...
	memcpy(&(work->tv_getwork_reply), &(work->tv_getwork), sizeof(struct timeval2));
...
static struct work *make_clone(struct work *work)
{
	...
	gettimeofday((struct timeval2 *)&(work_clone->tv_cloned), NULL);
...
{
	struct timeval2 now, then, tdiff;
...
static void hashmeter(int thr_id, struct timeval2 *diff,
		      uint64_t hashes_done)
{
	struct timeval2 temp_tv_end, total_diff;
	double secs;
	double local_secs;
	double utility;
	...
	if (thr_id >= 0) {
		...
		applog(LOG_DEBUG, "[thread %d: %I64u hashes, %.1f khash/sec]",
	...
	global_hashrate = floor(rolling+0.5) * 1000000;
...
{
	struct timeval2 tv_getwork, tv_getwork_reply;
	bool ret = false;
	json_t *val;
	CURL *curl;
	int rolltime;
...
retry_stratum:
	...
	if (val) {
		...
		if (rc) {
			...
			memcpy(&(work->tv_getwork), &tv_getwork, sizeof(struct timeval2));
			memcpy(&(work->tv_getwork_reply), &tv_getwork_reply, sizeof(struct timeval2));
...
void submit_work_async(struct work *work_in, struct timeval2 *tv_work_found)
{
	struct work *work = copy_work(work_in);
	pthread_t submit_thread;

	if (tv_work_found)
		memcpy(&(work->tv_work_found), tv_work_found, sizeof(struct timeval2));
...
static inline bool abandon_work(struct work *work, struct timeval2 *wdiff, uint64_t hashes)
{
...
static void hash_sole_work(struct thr_info *mythr)
{
	...
	struct timeval2 getwork_start, tv_start, tv_end, tv_workstart, tv_lastupdate;
	...
	struct timeval2 diff, sdiff, wdiff = {0, 0};
	...
	while (42) {
		...
		do {
			...
			if (diff.tv_sec > 0 || diff.tv_usec > 200) {
				...
				hashes_done = 0;
				memcpy(&tv_lastupdate, &tv_end, sizeof(struct timeval2));
			}
...
{
	struct timeval2 tv_start = {0, 0}, tv_end;
	struct cgpu_info *cgpu = mythr->cgpu;
	struct device_drv *drv = cgpu->drv;
	const int thr_id = mythr->id;
	int64_t hashes_done = 0;
	...
	while (42) {
		...
		if (diff.tv_sec > 0 || diff.tv_usec > 200) {
			hashmeter(thr_id, &diff, hashes_done);
			hashes_done = 0;
			memcpy(&tv_start, &tv_end, sizeof(struct timeval2));
		}
...
static void convert_to_work(json_t *val, int rolltime, struct pool *pool, struct timeval2 *tv_lp, struct timeval2 *tv_lp_reply)
{
	struct work *work;
	bool rc;
	...
	work->pool = pool;
	work->rolltime = rolltime;
	memcpy(&(work->tv_getwork), tv_lp, sizeof(struct timeval2));
	memcpy(&(work->tv_getwork_reply), tv_lp_reply, sizeof(struct timeval2));
	calc_diff(work, 0);
...
static void *longpoll_thread(void *userdata)
{
	 ...
	 * tied it to */
	struct timeval2 start, reply, end;
...
static struct timeval2 rotate_tv;
...
static void reap_curl(struct pool *pool)
{
	...
	for (ent = list_entry((&pool->curlring)->next, struct curl_ent, node), 
		 iter = list_entry(ent->node.next, struct curl_ent, node);	
	     &ent->node != (&pool->curlring); 					
	     ent = iter, iter = list_entry(iter->node.next, struct curl_ent, node))		
	{
		if (pool->curls < 2)
			break;
...
static void *watchdog_thread(void __maybe_unused *userdata)
{
	...
	struct timeval2 zero_tv;

	pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS, NULL);

	RenameThread("watchdog");

	memset(&zero_tv, 0, sizeof(struct timeval2));
	gettimeofday(&rotate_tv, NULL);
...
struct device_drv cpu_drv = {DRIVER_CPU,"CPU"};
#endif
...
int main(int argc, char *argv[])
{
	...
	strcpy(dirname,s);
	lendir=strlen(dirname);
	dirname[lendir-strlen(exefile)]='\0';

	strcpy(cgminer_path, dirname);
	free(s);
	strcat(cgminer_path, "/");
