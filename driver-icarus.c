...
static struct timeval2 history_sec = { HISTORY_SEC, 0 };
...
static int icarus_gets(unsigned char *buf, int fd, struct timeval2 *tv_finish, struct thr_info *thr, int read_count)
{
	ssize_t ret = 0;
	int rc = 0;
...
static bool icarus_detect_one(const char *devpath)
{
	...
	struct timeval2 tv_start, tv_finish;
	int fd;
				...
				__maybe_unused int64_t max_nonce)
{
	...
	uint32_t nonce;
	int64_t hash_count;
	struct timeval2 tv_start, tv_finish, elapsed;
	struct timeval2 tv_history_start, tv_history_finish;
	double Ti, Xi;
...
static struct api_data *icarus_api_stats(struct cgpu_info *cgpu)
{
	...
	root = api_add_timeval2(root, "history_time", &(info->history_time), false);
