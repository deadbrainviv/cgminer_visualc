				...
				int64_t __maybe_unused max_nonce)
{
	...
	if (gpu->dynamic && (++gpu->intervals * dynamic_us) > 70000) {
		...
		memcpy(&(gpu->tv_gpustart), &tv_gpuend, sizeof(struct timeval2));
		gpu->intervals = 0;
	}
...
struct device_drv opencl_drv={DRIVER_OPENCL,"opencl","GPU",
	opencl_detect,
...
/*
struct device_drv {
	enum drv_driver drv_id;	char *dname;	char *name;
