...
char *memmem(char *haystack, unsigned int haystack_len,
...
_clState *initCl(unsigned int gpu, char *name, size_t nameSize)
{
	...
	clState= (_clState *)calloc(1, sizeof(_clState));
	...
	source = (char *)file_contents(filename, &pl);
...
build:
	...
	if (status != CL_SUCCESS) {
		...
		logg = (char *)malloc(logSize);
		status = clGetProgramBuildInfo(clState->program, devices[gpu], CL_PROGRAM_BUILD_LOG, logSize, logg, NULL);
		applog(LOG_ERR, "%s", logg);
		return NULL;
	}
