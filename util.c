...
static size_t all_data_cb(const void *ptr, size_t size, size_t nmemb,
			  void *user_data)
{
	struct data_buffer *db = (struct data_buffer*) user_data;//np
	...
	newmem = (char*)realloc((void*)db->buf, newlen + 1);//np
	if (!newmem)
		return 0;

	db->buf = (char*)newmem; //np
	db->len = newlen;
	memcpy((struct data_buffer*)db->buf + oldlen, ptr, len);
	memcpy((struct data_buffer*)db->buf + newlen, &zero, 1);	/* null terminate */
...
static size_t upload_data_cb(void *ptr, size_t size, size_t nmemb,
			     void *user_data)
{
	struct upload_buffer *ub = (struct upload_buffer*) user_data;//np
...
void tq_free(struct thread_q *tq)
{
	...
	for (ent = list_entry((&tq->q)->next, struct tq_ent, q_node), 
...
double us_tdiff(struct timeval2 *end, struct timeval2 *start)
{
	return end->tv_sec * 1000000 + end->tv_usec - start->tv_sec * 1000000 - start->tv_usec;
}

/* Returns the seconds difference between end and start times as a double */
double tdiff(struct timeval2 *end, struct timeval2 *start)
{
